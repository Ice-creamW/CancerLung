...
@st.cache_data
def load_data():
    # แก้ไขชื่อไฟล์เป็น 'data.csv' เพื่อให้เรียกง่ายและลดข้อผิดพลาด
    try:
        df = pd.read_csv('data.csv')
        return df
    except FileNotFoundError:
        st.error("❌ ไม่พบไฟล์ 'data.csv' ใน GitHub ของคุณ กรุณาตรวจสอบชื่อไฟล์อีกครั้ง")
        return None
...
if df is not None:
    ...
    # ตรวจสอบว่ามีคอลัมน์เหล่านี้จริงก่อนลบ
